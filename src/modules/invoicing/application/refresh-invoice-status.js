...
function createRefreshInvoiceStatus({repository,hka,fiscalProvider=hka}){return{
  async execute({companyId,invoiceId}){
    ...
    let response;
    try{response=await fiscalProvider.status(companyId,{codigoSucursalEmisor:invoice.branch_code,numeroDocumentoFiscal:invoice.fiscal_number,puntoFacturacionFiscal:invoice.billing_point,tipoDocumento:invoice.document_type,tipoEmision:'01'},invoice.fiscalProvider||'hka');}
    catch(error){await repository.markFailure(companyId,invoice.id,'uncertain',error,reconciliationId);throw new ApplicationError('El proveedor fiscal no permitió confirmar el resultado; la factura continúa incierta.',{status:502,code:'INVOICE_STILL_UNCERTAIN'});}
    ...
    if(status==='uncertain')throw new ApplicationError('El proveedor fiscal aún no reporta un resultado terminal; la factura continúa incierta.',{status:502,code:'INVOICE_STILL_UNCERTAIN'});
