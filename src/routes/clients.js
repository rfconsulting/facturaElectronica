...
router.post('/import/zoho', requireAdministrator, verifyCsrf, upload.single('file'), async (req, res, next) => {
  ...
  try {
    ...
    const normalizeRuc=value=>String(value||'').toUpperCase().replace(/[^A-Z0-9]/g,'');const [existing] = await pool.execute('SELECT id,code,email,ruc FROM clients WHERE company_id=?', [req.company.id]);const seenRucs=new Map(existing.filter(row=>row.ruc).map(row=>[normalizeRuc(row.ruc),row]));const seenCodes=new Map(existing.filter(row=>row.code).map(row=>[row.code,row])),seenEmails=new Map(existing.filter(row=>row.email).map(row=>[row.email.toLowerCase(),row]));const duplicates=[],ready=[];for(const item of valid){const client=item.client,rucKey=normalizeRuc(client.ruc),duplicate=(rucKey&&seenRucs.get(rucKey))||(client.code&&seenCodes.get(client.code))||(client.email&&seenEmails.get(client.email.toLowerCase()));if(duplicate)duplicates.push({row:item.sourceRow,name:client.legalName,ruc:client.ruc,existingId:duplicate.id||null});else{ready.push(item);const marker={id:null};if(rucKey)seenRucs.set(rucKey,marker);if(client.code)seenCodes.set(client.code,marker);if(client.email)seenEmails.set(client.email.toLowerCase(),marker);}}
