...
test('actualiza estado mediante HKA y repara efectos autorizados de forma inyectable',async()=>{let saved,receivable;const repository={findStatusTarget:async()=>({id:4,status:'uncertain',created_by:3,customer_id:8,source_quote_id:6,opportunity_id:7,total:107,branch_code:'0001',billing_point:'001',document_type:'01',fiscal_number:'0000000004'}),beginReconciliation:async()=>true,saveRefreshedStatus:async(...args)=>{saved=args;},recordAuthorizedActivity:async()=>{},createReceivable:async input=>{receivable=input;}},useCase=createRefreshInvoiceStatus({repository,hka:{status:async()=>({estatusDocumento:'Autorizado',codigo:'200',secreto:'no exponer'})}}),result=await useCase.execute({companyId:2,invoiceId:4});assert.equal(result.body.status,'authorized');assert.deepEqual(saved.slice(0,3),[2,4,'authorized']);assert.equal(receivable.invoiceId,4);assert.equal(result.body.provider.secreto,undefined);});
