...
test('ERP expone tablero, cotizaciones, pedidos y cobros',()=>{const app=read('src/app.js'),routes=read('src/routes/erp.js'),ui=read('public/erp-ui.js'),bootstrap=read('public/js/bootstrap.js'),html=read('public/dashboard.html');assert.match(app,/\/api\/erp/);assert.match(routes,/router\.get\('\/dashboard'/);assert.match(routes,/router\.get\('\/orders'/);assert.match(ui,/Resumen ERP/);assert.match(ui,/Cotizaciones/);assert.match(ui,/Pedidos/);assert.match(ui,/Cuentas por cobrar/);assert.match(bootstrap,/erpDashboard/);assert.doesNotMatch(html,/<header class="area-hero crm-hero" data-area-navigation="erp">/);});

test('pedido conserva origen y prepara factura sin redirección HTTP',()=>{const routes=read('src/routes/erp.js'),invoiceRepository=read('src/modules/invoicing/infrastructure/invoice.repository.js');assert.match(routes,/source_quote_id AS sourceQuoteId/);assert.match(routes,/invoice-draft/);assert.match(routes,/sourceOrderId:order\.id/);assert.match(routes,/sales_order_items/);assert.doesNotMatch(routes,/res\.redirect/);assert.match(invoiceRepository,/UPDATE sales_orders SET status='invoiced'/);});
