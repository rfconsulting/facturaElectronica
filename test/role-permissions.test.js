...
test('la interfaz muestra importaciones a administradores y superusuarios',()=>{const bootstrap=fs.readFileSync(path.join(__dirname,'..','public/js/bootstrap.js'),'utf8'),html=fs.readFileSync(path.join(__dirname,'..','public/dashboard.html'),'utf8');assert.match(bootstrap,/role\s*===\s*'administrator'\s*\|\|\s*Boolean\(session\.user\.isSuperuser\)/);for(const id of ['import-hka-clients','import-clients','import-articles'])assert.match(html,new RegExp(`id="${id}"`));});
