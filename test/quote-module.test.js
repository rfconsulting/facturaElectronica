...
const {validateQuote}=require('../src/modules/quotations/application/quotation-validation');
...
test('cotizaciones tienen módulo vertical, historial y transiciones controladas',()=>{const root=path.join(__dirname,'..'),read=file=>fs.readFileSync(path.join(root,file),'utf8'),routes=read('src/modules/quotations/quotations.routes.js'),repository=read('src/modules/quotations/infrastructure/legacy-crm-quotes.repository.js'),schema=read('database/schema.sql');assert.match(routes,/controller\.transition/);assert.match(routes,/controller\.duplicate/);assert.match(repository,/Solo las cotizaciones en borrador pueden editarse/);assert.match(repository,/allowed=\{draft:/);assert.match(schema,/CREATE TABLE IF NOT EXISTS crm_quote_status_history/);});
