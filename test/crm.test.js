...
test('crear una oportunidad genera su próxima acción como tarea transaccional',()=>{const route=read('src/routes/crm-evolution.js');assert.match(route,/async function createNextActionTask/);assert.match(route,/INSERT INTO crm_tasks/);assert.match(route,/opportunityId:result\.insertId/);assert.match(route,/crm\.opportunity\.created/);assert.match(route,/taskId/);});

test('las tareas vinculadas son visibles en actividad y cotizaciones',()=>{const route=read('src/routes/crm.js'),ui=read('public/crm-ui.js');assert.match(route,/o\.title AS opportunityTitle/);assert.match(route,/t\.opportunity_id AS opportunityId/);assert.match(ui,/<h2>Tareas<\/h2>/);assert.match(ui,/t\.opportunityTitle\|\|t\.relatedName/);assert.match(ui,/data-task-complete/);});
